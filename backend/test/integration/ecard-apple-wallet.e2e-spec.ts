...
describe('E-card Add to Apple Wallet (e2e, TEST_DATABASE_URL only)', () => {
  ...
  async function seedCustomer(): Promise<{
    ...
    const email = `ecard-apple-wallet-e2e-${randomUUID()}@example.com`;
  ...
  describe('GET /api/ecards/me/:ecardId/wallet/apple', () => {
    it('returns 401 when unauthenticated', async () => {
      await request(app.getHttpServer())
        .get(`/api/ecards/me/${randomUUID()}/wallet/apple`)
        .expect(401);
    });
    ...
    it('fails gracefully (not a 500) and does not record a WALLET_SAVE event when Apple Wallet is not configured', async () => {
      const { customer, cookie } = await seedCustomer();
      const card = await prisma.eCard.create({
        data: {
          customerId: customer.id,
          endpoint: `ecard-apple-wallet-e2e-${randomUUID()}`,
          heroName: 'Test Customer',
          heroEmail: `ecard-apple-wallet-e2e-hero-${randomUUID()}@example.com`,
        },
      });

      const response = await request(app.getHttpServer())
        .get(`/api/ecards/me/${card.id}/wallet/apple`)
        .set('Cookie', cookie);
  ...
  describe('GET /api/employee/ecards/:id/wallet/apple', () => {
    it('returns 401 when unauthenticated', async () => {
      await request(app.getHttpServer())
        .get('/api/employee/ecards/some-id/wallet/apple')
        .expect(401);
    });
  });
});
