...
export const updateAddressSchema = z
  .object({
    label: z.string().trim().min(1).max(ADDRESS_LABEL_MAX_LENGTH).optional(),
    contactName: z
      .string()
      .trim()
      .min(1)
      .max(ADDRESS_CONTACT_NAME_MAX_LENGTH)
      .optional(),
    ...
    contactPhoneNumber: z
      ...
      .optional(),
    line1: z.string().trim().min(1).max(ADDRESS_LINE_MAX_LENGTH).optional(),
    line2: z.string().trim().max(ADDRESS_LINE_MAX_LENGTH).optional(),
    city: z.string().trim().min(1).max(ADDRESS_CITY_MAX_LENGTH).optional(),
    state: z.string().trim().min(1).max(ADDRESS_STATE_MAX_LENGTH).optional(),
    country: z
      .string()
      .trim()
      .min(1)
    ...
    pincode: z
      ...
      .trim()
      .min(1)
      .max(ADDRESS_PINCODE_MAX_LENGTH)
      .optional(),
    latitude: z
      .number()
