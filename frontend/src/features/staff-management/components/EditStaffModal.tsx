...
} from "@features/staff-management/schemas/editStaffSchema";
...
export default function EditStaffModal({
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditStaffValues>({ resolver: zodResolver(editStaffSchema) });
    ...
    <dialog ref={dialogRef} className="modal modal-bottom sm:modal-middle" onClose={onCancel}>
      <div className="modal-box">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-bold text-base-content">Edit staff</h3>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 flex flex-col gap-4">
          ...
          {canChangeRole && (
            <div>
              <label htmlFor="edit-staff-role" className="mb-1.5 block text-xs font-semibold text-base-content/70">
                Role
              </label>
              <select
                id="edit-staff-role"
