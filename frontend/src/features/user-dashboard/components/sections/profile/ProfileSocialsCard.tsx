...
export default function ProfileSocialsCard({ socials, onEdit }: ProfileSocialsCardProps) {
  ...
  function handleSocialClick(value: string) {
    if (!value) return;
  ...
  return (
    <div className="bg-base-100 rounded-2xl border border-base-300 shadow-sm p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-bold text-base-content">Socials Added</h2>
        <button
          type="button"
          onClick={onEdit}
      ...
      <div className="flex gap-6">
        {items.map(({ key, label, bgClass, icon }) => {
