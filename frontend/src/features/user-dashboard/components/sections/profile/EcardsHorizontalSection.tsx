...
import { getCustomerEcardGoogleWalletUrl } from "@services/customerEcardService";
...
export default function EcardsHorizontalSection({
  ...
  async function handleGoogleWallet(ecard: Ecard) {
    setWalletLoadingId(ecard.id);
    try {
      const { url } = await getCustomerEcardGoogleWalletUrl(ecard.id);
          ...
          {ecards.map((ecard) => (
            ...
            >
              ...
              >
                ...
                <button
                  type="button"
                  disabled={walletLoadingId === ecard.id}
                  onClick={() => void handleGoogleWallet(ecard)}
                  aria-label="Add to Google Wallet"
                  className="flex items-center justify-center px-3 py-2.5 text-base-content/35 hover:text-primary hover:bg-primary/8 transition-colors disabled:opacity-50"
                >
                  {walletLoadingId === ecard.id ? (
