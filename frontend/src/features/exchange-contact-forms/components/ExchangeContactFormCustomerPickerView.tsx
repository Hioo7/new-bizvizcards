...
import { adminCustomerExchangeContactFormsPath } from "@config/routes";

const CUSTOMER_PICKER_PAGE_SIZE = 20;
const CUSTOMER_PICKER_SEARCH_DEBOUNCE_MS = 350;

export default function ExchangeContactFormCustomerPickerView() {
  ...
  const {
    ...
    debounceMs: CUSTOMER_PICKER_SEARCH_DEBOUNCE_MS,
  });
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-8 sm:px-6">
      <div>
        <h1 className="text-2xl font-extrabold text-base-content">
          Exchange Contact Forms
        </h1>
        <p className="text-sm text-base-content/60">
          Pick a customer to build or manage their customizable forms.
        </p>
      </div>
        ...
        <div className="overflow-hidden rounded-box border border-base-300 bg-base-100">
          {customers.map((customer) => (
            <button
              ...
              type="button"
              onClick={() =>
                navigate(adminCustomerExchangeContactFormsPath(customer.id), {
                  state: { customer },
                })
              }
